/* affix.c XL prefix expression transformation */
...
statements()
{
	...
	if(match(SEMI)) {
		/* ensure the completness of the statement, 
		 * must end with SWMI(;).
		 */
		advance();
	} else {
	...
	if(!match(EOI))
    statements();		/* handle the next sentence */
}
...
char *expr_prime(char *exp)
{
	...
	if(match(PLUS) || match(MINUS)) {
		...
		/* +5 = +1(for char op) +2(for " ") +2(for "\0" string end after %s) */
...
char *factor()
{
	...
	if(match(NUM_OR_ID)) {
		...
		/* store the content of Token into fp */
