...
AST *make_abs(char *var, AST *body)
{
	AST *t = make_ast();
	t->kind = ABS;
	t->lchild = (AST *) var;		// why not use the union for AST* & char* ?
	t->rchild = body;
	return t;
}
...
void free_ast(AST *t)
{
	...
	switch (t->kind) {
		case CONST:
			/* only has kind & value, just free itself */
			break;
		case VAR:
			/* has kind & value, and lchild to save its name */
			if (t->lchild) {
				sfree(t->lchild);		// LEAF_NODE name
			} 
			break;
		case ABS: 
			if (t->lchild) {
				sfree(t->lchild);		// LEAF_NODE variable
			}
			free_ast(t->rchild);
			break;
		case COND:
			/* has all attr, no break */
			free_ast(t->cond);
		default:
			/* APP node */
			free_ast(t->lchild);
...
void print_expression(AST * tree, FILE *out) 
{
	...
	switch (tree->kind) {
		case CONST:
			/* value */
			fprintf(out, "%d", tree->value);
			break;
		case VAR: 
			/* (l:value(De Brujin index)) */
			fprintf(out, "(%s:%d)"
					, tree->lchild ? (char *) tree->lchild : "", tree->value);
			break;
		case COND:
			/* if_cond_then_l_else_r */
			fprintf(out, "if");
			print_expression(tree->cond, out);
			fprintf(out, "then");
			print_expression(tree->lchild, out);
			fprintf(out, "else");
