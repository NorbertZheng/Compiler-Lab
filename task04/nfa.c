...
LF_PTR lf_union(LF_PTR lf1, LF_PTR lf2)
{
	...
	while (tmp != NULL) {
		if (tmp->next == NULL) {
			// add to the tail of lf1
			tmp->next = lf2;
 			break;
		}
		// find the last element of lf1
		tmp = tmp -> next;
	}
	return lf1;
}
...
LF_PTR lf_union_plus(LF_PTR lf1, LF_PTR lf2)
{
	...
	while (lf2 != NULL) {
		...
		// no left common factor match with lf2's, just add it to the head
...
LF_PTR lf_concate(LF_PTR lf, AST_PTR exp)
{
	...
	if (tmp == NULL)
		return NULL;
	// while a1 is lf->symol, A is lf->exp, make AB to be lf->exp
	while (tmp != NULL) {
...
LF_PTR lf_concate_plus(LF_PTR lf, AST_PTR exp)
{
	...
	// use lf to return, tmp to visit, use lf instead of lf1 to avoid changing the structure of lf1
	tmp = lf = lf_clone(lf);
	if (tmp == NULL)
		return NULL;
	while (tmp != NULL) {
...
int is_minus(AST_PTR exp)
{
	...
	switch (exp->op) {
		...
		case Alpha:
			return 0;
		case Star:
			return is_minus(exp->lchild);
		default:
			return is_minus(exp->lchild) || is_minus(exp->rchild);
	}
}
...
{
	if (exp->state >= 0) {
		// means it has already been added
		return;
	}
	if (next_state % TABLEMAX == 0) {
		// overflow, reallocate StateTable, just a little bigger
		StateTable = safe_reallocate(StateTable, sizeof(exp) * (next_state + TABLEMAX));
	}
	// means exp has been recorded
...
void linear_form(AST_PTR exp, int stated)
{
	...
	if (exp->op == Epsilon) {
		...
		exp->nullable = 1;
		exp->lf = NULL;
	} else if (exp->op == Empty) {
		// set exp's attr
		exp->nullable = 0;
		exp->lf = NULL;
	} else if (exp->op == Alpha) {
		// set exp's attr
		exp->nullable = 0;
		exp->lf = mk_lf(exp->exp_string[0], EPSILON);
		// add state
...
static void enqueue_equiv(AST_PTR L, AST_PTR R) {
	...
	for (i = 0; i < equiv_next; i++) {
		if (L == EquivTable[i].left && R == EquivTable[i].right)
			return;
	}
	if (equiv_next == EQUIVTABLEMAX) {
		EQUIVTABLEMAX += TABLEMAX;
		EquivTable = safe_reallocate(EquivTable, sizeof(*EquivTable) * EQUIVTABLEMAX);
	}
		...
		for (j = i + 1; j < next_state; j++) {
			...
			for (current_queue = 0; current_queue < equiv_next; current_queue++) {
				...
				if (L->nullable != R->nullable)
					goto DISTINGUISHED;
				if (L->lf == NULL && R->lf != NULL)
					goto DISTINGUISHED;
				if (lf_length(L->lf) != lf_length(R->lf))
					goto DISTINGUISHED;
			...
			for (current_queue = 0; current_queue < equiv_next; current_queue++) {
				...
				if (EquivTable[current_queue].right->state > EquivTable[current_queue].left->state)
					EquivTable[current_queue].right->state = EquivTable[current_queue].left->state;
				else
					EquivTable[current_queue].left->state = EquivTable[current_queue].right->state;
			}
DISTINGUISHED:
			continue;
		}
...
void printnfa(char Q_or_D)
{
	...
	for (i = 1; i < next_state; i++) {
		...
		while (lf_tmp != NULL) {
			q = lf_tmp->exp->state;
			if (q)
				printf("| %c %c%d ", lf_tmp->symbol, Q_or_D, q);
			lf_tmp = lf_tmp->next;	 
		}
...
void graphviz_nfa(char *filename, char Q_or_D)
{
	...
	fprintf(gv_file, "digraph\n  G { label=\"%s of %s\";\n \nrankdir=LR\nstart[shape =none]\nstart ->node_1\n", Q_or_D == 'Q'? "NFA": (Q_or_D == 'D'? "DFA": "MDFA"), StateTable[1]->exp_string);
